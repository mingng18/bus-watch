...
## 2026-05-24 - Test function against actual codebase not issue description
**Learning:** Issue descriptions might contain simplified or incorrect code snippets compared to the actual codebase (e.g. `fetchGtfsRealtime` vs `fetchVehiclePositions`).
**Action:** Always verify the actual function signatures and logic in the codebase (using `grep` or `cat`) and write tests against the real code, not the hypothetical code in the issue description.
