...
**Action:** Modified `APIClient` private initializer to internal with a `session: URLSession` parameter defaulting to `.shared`, and created `APIClientMockURLProtocol` to assert requests and provide stubbed JSON responses or errors in tests.
