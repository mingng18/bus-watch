...
## 2025-05-15 - Replace text abbreviations with standard icons and expanded text
**Learning:** In SwiftUI, avoid using raw text abbreviations (e.g., 'sched') for domain-specific data states. These are visually unappealing and not immediately understandable for all users.
**Action:** Use `Label` components with expanded text and standard SF Symbols (e.g., 'clock') to improve visual comprehension and accessibility without taking up much more space. If embedding in strings, prefer `Text` interpolation with `Image(systemName:)`.
