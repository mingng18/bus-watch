...
## 2025-02-12 - [Performance/Security] 🛡️ Object.create(null) for safe, fast deduplication
**Learning:** When optimizing array deduplication by replacing `Set` with a plain object dictionary in JavaScript/TypeScript, using `{}` introduces prototype collision risks if keys like `__proto__` or `constructor` are present in external data. Using `Object.create(null)` creates a "clean" dictionary without inheriting from `Object.prototype`, making it both safe from prototype pollution and consistently faster than `Set` due to reduced allocation overhead in V8.
**Action:** When tracking seen strings or identifiers in hot code paths, use `const seen = Object.create(null) as Record<string, boolean>;` instead of `new Set()` or a standard `{}`. This ensures safety for arbitrary string keys while maintaining the performance benefits of a simple hash map structure.
