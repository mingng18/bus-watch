...
## 2025-02-28 - Optimize string sorting with direct operators
**Learning:** Avoid using `String.prototype.localeCompare` to sort strictly formatted ASCII strings (like `HH:MM:SS` departure times). It applies complex internationalization collation rules that introduce noticeable performance overhead. In JavaScript, simple comparison operators (`a < b ? -1 : a > b ? 1 : 0`) provide much faster lexicographical sorting for these formats.
**Action:** Replaced `localeCompare` in `backend/src/departures-toward.ts` and `backend/src/station.ts` with direct operators, yielding noticeably faster sorting during hot-path departure aggregation.
