...
## 2024-10-25 - Avoid localeCompare for strict ASCII strings
**Learning:** Using `String.prototype.localeCompare` to sort strictly formatted ASCII strings (like `HH:MM:SS` times in GTFS departures) applies complex internationalization collation rules that introduce noticeable performance overhead (~58% slower in a tight loop of 100k sorts).
**Action:** When sorting strictly formatted data like times where lexicographical sorting is sufficient, use simple comparison operators (`a < b ? -1 : a > b ? 1 : 0`) instead of `localeCompare` to reduce execution time.
