...
## 2025-02-28 - Avoid localeCompare for basic string sorting
**Learning:** Using `String.prototype.localeCompare` to sort strictly formatted ASCII strings (like `HH:MM:SS` times) applies complex internationalization collation rules that introduce noticeable CPU overhead.
**Action:** Instead, use simple comparison operators (`a < b ? -1 : a > b ? 1 : 0`) for much faster lexicographical sorting on formatted fields like timestamps.
