...
## 2024-07-28 - [Performance] ⚡ Dynamic Bounding Box Pruning
**Learning:** When evaluating distance across large coordinate arrays, repeatedly performing O(N) array scans to find the nearest point (like `nearestFromStopOnRoute`), recalculating the precise `haversineDistance` for every single coordinate introduces significant trigonometric overhead. Additionally, calling generic bounding box helpers inside the loop repeats constant calculations (like `Math.cos(lat)`).
...
## 2024-07-28 - [Performance] ⚡ Two-phase Promise.all fetching
...
## 2024-08-04 - [Performance] ⚡ Async Data Fetching Parallelization
**Learning:** Sequential `await` calls for independent data sets (like configuration, agency data, or vehicle positions) accumulate latency linearly, blocking subsequent network operations unnecessarily.
**Action:** Always wrap independent async operations in `Promise.all()` arrays. This guarantees network requests execute concurrently, bonding the total delay to the longest single request rather than the sum of all requests, drastically cutting endpoint response times.
...
## 2024-09-15 - [Performance] ⚡ Replace `.some` with `for` loops in hot UI paths
**Learning:** Using `.some()` with inline lambdas to check boolean array conditions inside SwiftUI view body evaluation or highly repeated data processing pipelines generates unnecessary closure allocations per run.
**Action:** Replaced `routes.some(x => x === target)` patterns with direct `for` loop evaluations to eliminate lambda allocation overhead, especially in code that executes frequently or is deeply nested.
