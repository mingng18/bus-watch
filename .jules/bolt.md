...
## 2024-08-01 - Avoid Scope Creep during PR updates
**Learning:** If a code review bot hallucinates scope creep or merge conflicts that are not actually present in `git diff origin/master..HEAD`, treating the feedback as literal truth and performing massive rebases or switching branches can cause actual state corruption.
**Action:** Always verify the diff using `git diff origin/master..HEAD`. If the diff correctly contains ONLY your targeted optimization, but the reviewer bot claims otherwise, it is hallucinating. Reply to the PR comment clarifying the exact clean state of the branch and proceed with submission, rather than entering a destructive rebase loop.
