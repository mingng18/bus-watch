...
## $(date +%Y-%m-%d) - Missing Cache-Control on Authenticated Endpoints
**Vulnerability:** Authenticated administrative endpoints (like `/refresh` and `/rail/ingest`) lacked explicitly restrictive cache directives.
**Learning:** By default, responses generated by endpoints behind authentication middleware may be cached by intermediate proxies, CDNs, or the user's browser unless specifically instructed not to. This can inadvertently expose sensitive data or administrative responses to unauthorized access or cache poisoning.
**Prevention:** Always add a `Cache-Control: no-store` header to responses from authenticated or administrative endpoints to explicitly instruct all layers to bypass caching.
