...
## 2025-02-27 - Configure timeouts on external fetch calls to prevent serverless hanging DoS
**Vulnerability:** External `fetch` requests without timeouts can hang indefinitely if the upstream service becomes unresponsive. In serverless environments, this can tie up concurrency limits and cause Denial of Service (DoS).
**Learning:** `AbortSignal.timeout(ms)` provides a built-in way to abort hanging fetch requests. Serverless applications must use timeouts for all external requests to ensure predictable execution limits and prevent unbounded resource consumption.
**Prevention:** Always pass `{ signal: AbortSignal.timeout(ms) }` as part of options to `fetch` calls. Ensure that the associated logic wraps the fetch in a `try...catch` block to handle the potential `TimeoutError` without crashing the application.
