...
## 2024-07-05 - Fix overly permissive CORS configuration
**Vulnerability:** The Hono CORS middleware used a fallback origin (`'http://localhost:8081'`) when `FRONTEND_URL` was undefined. This would allow an attacker running on that port to bypass CORS policy.
**Learning:** Hardcoded localhost origins in fallback configurations are dangerous because attackers can host local instances mimicking those addresses.
**Prevention:** If an environment variable isn't present, the fallback should either be omitted/empty (denying cross-origin access by default) or scoped to explicit dev environments (e.g. checking an `ENVIRONMENT` flag).
