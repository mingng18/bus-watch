...
export function findNearbyStops(
  ...
  // Optimization: Precompute maps for O(1) lookups instead of doing O(N) operations repeatedly
...
export function findNearbyBusRoutes(
  ...
  const routeMap = new Map(routes.map(r => [r.id, r]));
  // Optimization: Precompute map for O(1) trip lookups
  const tripMap = new Map(trips.map(t => [t.id, t]));
...
export function findNearbyPrasaranaBuses(
  ...
  // Optimization: Precompute map by routeId for O(1) lookups instead of O(N) trips.find() per route
  const tripsByRouteId = new Map<string, Trip>();
  for (const t of trips) {
    if (!tripsByRouteId.has(t.routeId)) {
      tripsByRouteId.set(t.routeId, t);
    }
  }

  for (const r of routes) {
    if (!routeNameMap.has(r.shortName)) {
      const trip = tripsByRouteId.get(r.id);
