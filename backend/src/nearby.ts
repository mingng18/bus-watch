...
export function findNearbyStops(
  ...
  // ⚡ Optimization: Precompute Maps outside of loops to convert O(N^2) lookups to O(N) operations
...
export function findNearbyBusRoutes(
  ...
  // ⚡ Optimization: Precompute routeMap and tripMap to avoid O(N^2) lookups during vehicle iteration
  const routeMap = new Map(routes.map((r) => [r.id, r]));
  const tripMap = new Map(trips.map((t) => [t.id, t]));
...
export function findNearbyPrasaranaBuses(
  ...
  // ⚡ Optimization: Precompute a Map of the first trip per routeId to avoid an O(N) array scan per route
