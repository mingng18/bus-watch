...
const staticCache = new Map<
  string,
  { ts: number; dataPromise: Promise<any> }
>();
...
const ROUTES_KEYS = [...AGENCIES, ...SELANGOR_AGENCIES].map(
  (a) => `routes:${a}`,
);
...
async function getCachedStaticData<T>(
  kv: KVNamespace,
  cacheKey: string,
  keys: string[],
  isArray = true,
  ...
  const dataPromise = Promise.all(
    ...
    return (
      isArray ? results.flatMap((r) => r || []) : Object.assign({}, ...results)
    ) as T;
  });
...
async function getRoutesMaps(
  kv: KVNamespace,
): Promise<{ map: Map<string, Route>; shortNameMap: Map<string, Route> }> {
  const now = Date.now();
  ...
  for (let i = 0; i < allRoutes.length; i++) {
    ...
    if (r.shortName && !shortNameMap.has(r.shortName))
...
async function getAllTripStops(kv: KVNamespace) {
  return getCachedStaticData<Record<string, any[]>>(
    kv,
    "tripStops",
    TRIP_STOPS_KEYS,
    false,
...
async function getAllShapes(kv: KVNamespace) {
  return getCachedStaticData<Record<string, [number, number][]>>(
    kv,
    "shapes",
    SHAPES_KEYS,
    false,
