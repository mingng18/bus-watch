  ...
  for (const stop of result) {
    if (stop.type === "bus") {
      for (const arrival of stop.arrivals) {
        if (arrival.route) {
          const eta = await getHistoricalETA(
            c.env.DB,
            arrival.route,
            0,
            0,
...
app.get("/bus/eta", async (c) => {
  ...
  try {
    ...
    console.error("Error fetching ETA:", err?.message || "Unknown error");
...
app.get("/route/:routeId", async (c) => {
  ...
  if (shapes.length === 0) {
    try {
      ...
      console.error(
        "Failed to reconstruct route shape from D1:",
  ...
  scheduled: async (event: ScheduledEvent, env: Env, ctx: ExecutionContext) => {
    if (event.cron === "*/5 * * * *") {
      ...
      try {
        ...
        console.error(
          "Failed to fetch realtime data for sampling:",
          err?.message || "Unknown error",
        );
      }
      ...
      try {
        ...
        console.error(
          "Failed to ingest rail timetables:",
          err?.message || "Unknown error",
        );
      }
    } else {
      ...
      try {
        ...
        console.error(
          "Failed to refresh Prasarana buses:",
          err?.message || "Unknown error",
        );
      }
    }
