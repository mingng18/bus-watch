...
function parseStops(
  ...
  const routeIdToType = new Map(
    rawRoutes.map((r) => [r.route_id, r.route_type]),
  );
  const tripToRouteType = new Map(
    rawTrips.map((t) => [t.trip_id, routeIdToType.get(t.route_id) || "3"]),
  );
...
function parseTripStops(
  rawStopTimes: GtfsStopTime[],
  stopMap: Map<string, Stop>,
): Record<string, TripStopEntry[]> {
...
function parseCalendar(rawCalendar: GtfsCalendar[]): CalendarEntry[] {
  return rawCalendar.map((c) => ({
...
export async function fetchAndParseAgency(agency: string): Promise<AgencyData> {
  ...
  const rawStopTimes = parseCsv(
    getFile("stop_times.txt"),
  ) as unknown as GtfsStopTime[];
  const rawCalendar = parseCsv(
    getFile("calendar.txt"),
  ) as unknown as GtfsCalendar[];

  const { stops, stopMap } = parseStops(
    rawStops,
    rawRoutes,
    rawTrips,
    rawStopTimes,
