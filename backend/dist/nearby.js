...
export function findNearbyStops(stops, routes, trips, tripStops, calendar, frequencies, vehicles, lat, lon, radiusM) {
    const now = new Date();
    const nearby = stops
        .map((stop) => ({
        stop,
...
export function findNearbyBusRoutes(routes, trips, vehicles, lat, lon, radiusM = 1000) {
    const routeMap = new Map(routes.map((r) => [r.id, r]));
    const tripMap = new Map(trips.map((t) => [t.id, t]));
    const results = [];
    const seen = new Set();
...
export function findNearbyPrasaranaBuses(buses, routes, trips, lat, lon, radiusM = 1000) {
    const routeTripMap = new Map();
...
export async function getHistoricalETA(db, route, fromLat, fromLon, toStopId) {
    const { results } = await db
        .prepare(`SELECT * FROM travel_times WHERE route = ? AND to_stop_id = ? LIMIT 1`)
        .bind(route, toStopId)
        .all();
