...
app.get('/route/:routeId', async (c) => {
    ...
    if (shapes.length === 0) {
        try {
            ...
            const { results: posRows } = await c.env.DB.prepare(`SELECT bus_no, lat, lon, timestamp FROM bus_positions
         WHERE route = ? OR route = ?
         ORDER BY bus_no, timestamp`).bind(routeNormal, routeSuffixed).all();
            if (posRows && posRows.length > 0) {
                // Group by bus_no to get per-bus traces
                const groups = new Map();
                for (const row of posRows) {
                    if (!groups.has(row.bus_no))
                        groups.set(row.bus_no, []);
