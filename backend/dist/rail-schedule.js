...
export async function getRailSchedule(env, stopId, windowMinutes = 120) {
    // 1. Fetch stop info
    const stopRow = await env.DB.prepare(`SELECT stop_id, stop_name FROM rail_stops WHERE stop_id = ?`).bind(stopId).first();
    if (!stopRow)
        return null;
    ...
    const { results } = await env.DB.prepare(`SELECT
     ...
     LIMIT 20`).bind(stopId, currentMinutes, upperMinutes).all();
    ...
    // 4. Check staleness
    const metaRow = await env.DB.prepare(`SELECT value FROM rail_ingest_meta WHERE key = 'last_ingested_at'`).first();
    let stale = false;
    if (metaRow) {
 ...
 */
export async function searchRailStops(env, query) {
    const { results } = await env.DB.prepare(`SELECT stop_id, stop_name, lat, lon
     FROM rail_stops
     WHERE LOWER(stop_name) LIKE LOWER(?)
     ORDER BY stop_name ASC
     LIMIT 20`).bind(`%${query}%`).all();
    return results;
}
