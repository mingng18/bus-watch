...
const FIXTURE = readFileSync(join(__dirname, 'fixtures/myrapid-sitemap.xml'), 'utf8');
...
describe('parseAlerts', () => {
  it('extracts and normalizes the known disruption patterns from the fixture', () => {
    ...
    expect(closure!.affectedLines).toEqual(['151', '171', '173', '180', '190', '202', '220', '250']);
    expect(closure!.title).toBe('Road closure — routes 151, 171, 173, 180, 190, 202, 220, 250');
    ...
    const lineUpdate = alerts.find((a) => a.id.startsWith('kemas-kini-laluan-ampang'));
  ...
  it('excludes the homepage and non-disruption posts (events/promos)', () => {
    const alerts = parseAlerts(FIXTURE);
    expect(alerts.some((a) => a.id === 'malaysian-philharmonic-orchestra')).toBe(false);
...
describe('parseAlerts — robustness', () => {
  ...
  it('returns [] for input with no <url> blocks', () => {
    expect(parseAlerts('<?xml version="1.0"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"></urlset>')).toEqual([]);
  });

  it('tolerates malformed / truncated XML without throwing', () => {
    const malformed = '<urlset><url><loc>https://myrapid.com.my/info-penutupan-jalan-laluan-100-9/</loc><lastmod>not-a-date</lastmod></url>'; // no closing tags
...
describe('fetchAlerts', () => {
  ...
  it('returns [] (does not throw) when the source returns a non-200 status', async () => {
    ...
    const env = makeEnv(makeKV());
    expect(await fetchAlerts(env)).toEqual([]);
  });
...
describe('getCachedAlerts', () => {
  ...
  it('serves the cached list without fetching while within the TTL', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const kv = makeKV();
    const cachedAlerts = [{ id: 'cached-1', title: 'Cached', summary: 'Cached', date: '2026-06-13T00:00:00.000Z', affectedLines: [], severity: 'info', url: 'https://myrapid.com.my/cached' }];
  ...
  it('re-fetches and re-caches after the TTL elapses', async () => {
    ...
    kv.store.set(ALERTS_KV_KEY, { ts: Date.now() - ALERTS_CACHE_TTL_MS - 1, alerts: [] });
  ...
  it('serves stale cache when a fresh fetch comes back empty', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const kv = makeKV();
    const staleAlerts = [{ id: 'stale-1', title: 'Stale', summary: 'Stale', date: '2026-06-01T00:00:00.000Z', affectedLines: [], severity: 'warning', url: 'https://myrapid.com.my/stale' }];
    kv.store.set(ALERTS_KV_KEY, { ts: Date.now() - ALERTS_CACHE_TTL_MS - 1, alerts: staleAlerts });
