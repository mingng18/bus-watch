...
import { getHistoricalETA, getBatchedHistoricalETAs, confidenceFromSamples, nearestFromStopOnRoute } from '../src/nearby';
...
const STOP_A: TripStopEntry = { stopId: 'A', stopName: 'A', lat: 3.1300, lon: 101.6800, arrivalTime: '', departureTime: '', sequence: 1 };
const STOP_B: TripStopEntry = { stopId: 'B', stopName: 'B', lat: 3.1357, lon: 101.6800, arrivalTime: '', departureTime: '', sequence: 2 };
const STOP_C: TripStopEntry = { stopId: 'C', stopName: 'C', lat: 3.1414, lon: 101.6800, arrivalTime: '', departureTime: '', sequence: 3 };
...
describe('detectStopPassages', () => {
  it('detects a clean A→B→C passage and emits two inter-stop samples', () => {
    ...
    const samples: PositionSample[] = [
      { bus_no: 'b1', route: 'r1', lat: 3.1300, lon: 101.6800, timestamp: T0 },
      { bus_no: 'b1', route: 'r1', lat: 3.1357, lon: 101.6800, timestamp: T0 + 120 },
      { bus_no: 'b1', route: 'r1', lat: 3.1414, lon: 101.6800, timestamp: T0 + 240 },
    ];
    const legs = detectStopPassages(samples, SEQ_ABC, 'r1');
    expect(legs).toHaveLength(2);
    expect(legs[0]).toMatchObject({ route: 'r1', from_stop_id: 'A', to_stop_id: 'B', seconds: 120 });
    expect(legs[1]).toMatchObject({ route: 'r1', from_stop_id: 'B', to_stop_id: 'C', seconds: 120 });
  ...
  it('drops the leg across a skipped stop without fabricating it', () => {
    ...
    const samples: PositionSample[] = [
      { bus_no: 'b1', route: 'r1', lat: 3.1300, lon: 101.6800, timestamp: T0 },
      { bus_no: 'b1', route: 'r1', lat: 3.1414, lon: 101.6800, timestamp: T0 + 200 },
    ];
    const legs = detectStopPassages(samples, SEQ_ABC, 'r1');
  ...
  it('ignores a far GPS outlier that is not within radius of the next stop', () => {
    ...
    const samples: PositionSample[] = [
      { bus_no: 'b1', route: 'r1', lat: 3.1300, lon: 101.6800, timestamp: T0 },
      { bus_no: 'b1', route: 'r1', lat: 3.5000, lon: 102.0000, timestamp: T0 + 60 }, // outlier
      { bus_no: 'b1', route: 'r1', lat: 3.1357, lon: 101.6800, timestamp: T0 + 120 },
    ];
    const legs = detectStopPassages(samples, SEQ_ABC, 'r1');
    expect(legs).toHaveLength(1);
    expect(legs[0]).toMatchObject({ from_stop_id: 'A', to_stop_id: 'B', seconds: 120 });
  });
  ...
  it('caps absurd inter-stop gaps (out-of-service) and emits no sample for that leg', () => {
    ...
    const samples: PositionSample[] = [
      { bus_no: 'b1', route: 'r1', lat: 3.1300, lon: 101.6800, timestamp: T0 },
      { bus_no: 'b1', route: 'r1', lat: 3.1357, lon: 101.6800, timestamp: T0 + 7200 },
      { bus_no: 'b1', route: 'r1', lat: 3.1414, lon: 101.6800, timestamp: T0 + 7320 },
    ];
    const legs = detectStopPassages(samples, SEQ_ABC, 'r1');
    // The A→B leg (7200s) is dropped; B→C (120s) survives.
    expect(legs).toHaveLength(1);
    expect(legs[0]).toMatchObject({ from_stop_id: 'B', to_stop_id: 'C', seconds: 120 });
  });
  ...
  it('returns no samples for an empty trace or a single-stop sequence', () => {
    ...
    expect(detectStopPassages([STOP_A].map(s => ({ bus_no: 'b1', route: 'r1', lat: s.lat, lon: s.lon, timestamp: T0 })), [STOP_A], 'r1')).toEqual([]);
  });
...
describe('aggregateSamples', () => {
  it('averages samples per (route, from, to, day, hour) bucket and counts them', () => {
    ...
    expect(out[0]).toMatchObject({ route: 'r1', from_stop_id: 'A', to_stop_id: 'B', day_of_week: 5, time_bucket: 9 });
  ...
  it('does not reject anything when there are too few samples', () => {
    const samples = [leg('r1', 'A', 'B', 5, 9, 100), leg('r1', 'A', 'B', 5, 9, 1500)];
    const out = aggregateSamples(samples);
...
describe('nearestFromStopOnRoute', () => {
  it('returns the stop closest to the bus position', () => {
    ...
    const stop = nearestFromStopOnRoute(3.1330, 101.6800, SEQ_ABC);
...
describe('getHistoricalETA', () => {
  it('returns ETA + confidence + isLive for a known leg', async () => {
    const { db } = mockDb({ avg_seconds: 300, sample_count: 10, spread_seconds: 45 });
...
describe('getBatchedHistoricalETAs', () => {
  it('returns a map keyed by route-stopId with the confidence result', async () => {
    const { db } = mockDb({ avg_seconds: 480, sample_count: 4, spread_seconds: 60 });
    const map = await getBatchedHistoricalETAs(db, [{ route: 'r1', stopId: 'B' }]);
...
// ---- helper ----------------------------------------------------------------
function leg(route: string, from: string, to: string, dow: number, hour: number, seconds: number) {
  return {
    route,
    from_stop_id: from,
    to_stop_id: to,
