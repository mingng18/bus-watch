...
import { Stop, Route, Trip, VehiclePosition, TripStopEntry, CalendarEntry, Frequency } from '../src/types';
...
const tripStops: Record<string, TripStopEntry[]> = {
  t1: [
    { stopId: 's1', stopName: 'Bangsar LRT', lat: 3.1295, lon: 101.6750, arrivalTime: '22:30:00', departureTime: '22:30:00', sequence: 1 }
  ]
};

const calendar: CalendarEntry[] = [
  { serviceId: 'weekday', days: [true, true, true, true, true, true, true], startDate: '20000101', endDate: '20991231' }
];
...
describe('findNearbyStops', () => {
  ...
  it('includes rail scheduled arrivals', () => {
    // Note: this test might be flaky depending on the actual time it runs, as it relies on 'now'
    // For a robust test, expandTripsForStop/getStationSchedule would need mockable time.
    // Given the current implementation of expandTripsForStop, we'll just check if it runs without crashing.
    const result = findNearbyStops(stops, routes, trips, tripStops, calendar, frequencies, vehicles, 3.1290, 101.6755, 500);
