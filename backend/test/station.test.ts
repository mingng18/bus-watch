...
describe("getStationSchedule", () => {
  ...
  it("returns departures sorted by time", () => {
    ...
    expect(
      result.departures[0].departureTime.localeCompare(
        result.departures[1].departureTime,
      ),
  ...
  it("throws for unknown stop", () => {
    expect(() =>
      getStationSchedule("unknown", stops, routes, trips, tripStops, calendar),
    ).toThrow();
  });
