...
describe("analyzeFeed — vehicle-position", () => {
  it("counts occupancy / congestion / percentage population and names distributions", () => {
    ...
    expect(report.vehicle!.occupancyPercentage).toEqual({
      total: 2,
      populated: 1,
    });
    expect(report.vehicle!.occupancyStatusCounts["MANY_SEATS_AVAILABLE"]).toBe(
      1,
    );
    ...
    expect(
      report.vehicle!.congestionLevelCounts["UNKNOWN_CONGESTION_LEVEL"],
    ).toBe(1);
  });
});
      ...
      entity: [
        {
          ...
          alert: {
            ...
            descriptionText: {
              translation: [{ text: "Delay due to weather" }],
            },
            cause: 8, // WEATHER
