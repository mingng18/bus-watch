...
export function analyzeFeed(
  ...
  if (entity === "vehicle-position") {
    ...
    report.alert = {
      ...
      descriptionText: countField(
        alerts,
        (a) => !!translatedText(a.descriptionText),
      ),
      ttsHeaderText: countField(
        alerts,
        (a) => !!translatedText(a.ttsHeaderText),
      ),
      ttsDescriptionText: countField(
        alerts,
        (a) => !!translatedText(a.ttsDescriptionText),
      ),
      ...
      informedEntity: countField(
        alerts,
        (a) => !!a.informedEntity && a.informedEntity.length > 0,
      ),
      causeCounts: tally(
        alerts.map((a) => (a.cause as unknown as number) ?? null),
        CAUSE_NAMES,
      ),
      effectCounts: tally(
        alerts.map((a) => (a.effect as unknown as number) ?? null),
        EFFECT_NAMES,
      ),
    ...
    report.tripUpdate = {
      ...
      withStopTimeUpdate: countField(
        updates,
        (t) => !!t.stopTimeUpdate && t.stopTimeUpdate.length > 0,
      ),
      ...
      sampleRouteIds: Array.from(
        new Set(
          updates.map((t) => t.trip?.routeId).filter((r): r is string => !!r),
        ),
      ).slice(0, 10),
    };
  }
...
function formatReport(c: FeedCandidate, r: FeedReport): string {
  ...
  if (r.vehicle) {
    ...
    lines.push(
      `  occupancyStatus distribution: ${JSON.stringify(v.occupancyStatusCounts)}`,
    );
    lines.push(
      `  congestionLevel distribution: ${JSON.stringify(v.congestionLevelCounts)}`,
    );
  }
  if (r.alert) {
    const a = r.alert;
    ...
    lines.push(
      `  severity distribution:   ${JSON.stringify(a.severityCounts)}`,
    );
...
async function main(): Promise<void> {
  ...
  if (sampleArg)
    console.log(
      `(note: --sample only truncates display; decoding is whole-feed)`,
    );
